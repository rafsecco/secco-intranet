...
description: Notação húngara de banco de dados padrão de Rafael (originada na Secco Platform, ADR-0017), aplicável a qualquer projeto .NET dele, não só os com prefixo Secco.*/RS.*. Usar SEMPRE que a tarefa envolver criação ou alteração de schema de banco, entidades EF Core, configurações de mapeamento (IEntityTypeConfiguration), migrations, scripts SQL, views, índices, constraints, procedures ou functions — em qualquer projeto .NET, ou quando o usuário mencionar prefixos como tb_, id_, ds_, dt_, fl_, notação húngara de banco, ou nomenclatura de tabelas/colunas.
---
